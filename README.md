...
#### A. 논문이 먼저 있다 — "교수님이 이 논문 읽고 우리 데이터에 적용해보래" *(랩 연구 · 아이디어 확장)*

```
/potato-lit-review    선행·후속 지형 파악
     ↓
/potato-study-paper   (옵션) 논문 한 편을 처음부터 정독
     ↓                 — 번역·해설·수식이 브라우저 HTML로 쌓인다
...
**핵심**: 재현 수치가 안 맞으면 원인부터 규명합니다.
안 그러면 "내 아이디어가 좋아서 오른 건지" 알 수 없어요.

#### B. 데이터가 이미 있다 — "이 데이터로 뭘 예측/분류하고 싶다" *(해커톤 · 캐글 · 데이콘 · 과제)*
...
**핵심**: 감사를 건너뛰지 마세요. 누수 하나로 리더보드 1등이 실격됩니다.

#### C. 주제만 있다 — "청년 정주여건 개선하래… 데이터도 문제도 내가 정해야 해" *(지자체 과제 · 기관 프로젝트 · 자유주제)*
...
**핵심**: 수집을 **스크립트로** 남깁니다. 보고서의 "데이터 출처" 절이 저절로 채워져요.

#### D. 결정을 내려야 한다 — "근무표를 짜야 해" "창구 몇 개면 될까" *(산업공학 · 운영 개선)*
...
**핵심**: 최적화의 실패는 푸는 단계가 아니라 **세우는 단계**에서 일어납니다.
