...
> `GroupKFold`가 아니라 `StratifiedGroupKFold`를 쓴다. 전자는 **층화를 하지 않아**
> 폴드별 타깃 비율이 흔들린다(scikit-learn 1.6부터 shuffle 옵션은 생겼지만 층화는
> 여전히 없다). `StratifiedKFold` 결과와 비교할 때 그 차이를 "그룹 누수"로 오해하게 된다.
...
- 표준편차를 반드시 함께 적는다. 차이가 편차 안이면 "차이 있다"고 말하지 않는다
  (폴드 std는 보수적 참고용 — 순위 주장은 다중 시드 반복으로 확인한 뒤에).
